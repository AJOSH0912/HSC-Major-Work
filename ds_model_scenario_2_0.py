...
def create_team_stats(df):
    ...
    home_df = df.copy()
    home_df['match_id'] = home_df['match_id'] #Creates a new column called match_id with the same value from the original dataframe
...
# Creates the columns for the average of the last 10 matches for each team
avg_last10_columns = [
...
rolling_average_last5_home.columns = rolling_average_last5_home.columns[:3].tolist() + ['team_1_'+str(col) for col in rolling_average_last5_home.columns[3:]] # Renames the columns to include team_1_ for the away team for when we merge later

rolling_average_last5_away.columns = rolling_average_last5_away.columns[:3].tolist() + ['team_2_'+str(col) for col in rolling_average_last5_away.columns[3:]] # Renames the columns to include team_2_ for the away team for when we merge later


Last5_Home_Away = pd.merge( rolling_average_last5_home, rolling_average_last5_away, on=['match_id' , 'date'], how='left') # Merges the last 5 home and away matches dataframes on team, date and match_id

Last5_Home_Away.to_csv('data/Last5_Home_Away.csv', index=False) # Saves the last 5 home and away matches to a CSV file

home_team = rolling_average_last10[rolling_average_last10['home_away'] == 'H'].copy() # Creates a copy of the rolling average last 10 matches for home teams so that we can put them side to side again with pd merge
away_team = rolling_average_last10[rolling_average_last10['home_away'] == 'A'].copy() # Creates a copy of the rolling average last 10 matches for away teams so that we can put them side to side again with pd merge
...
Combined_data = pd.merge(Last5_Home_Away, last10_stats, on=['match_id', 'date'], how='left') # Merges the last 5 home and away matches with the last 10 home and away matches on team, date and match_id
